...
async function getNormalizedMonsters() {
  ...
  const normalizedData = monsterDetails.map((monster) => {
    ...
    const speedValues = monster.speed
      ? Object.values(monster.speed).map((s) => parseInt(s))
      : [0];
...
getNormalizedMonsters().then((data) => console.log(data));
